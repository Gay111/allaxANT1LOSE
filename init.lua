...
-- Первичная регистрация биндов в микро-HUD
...
-- // 1. ВКЛАДКА: AIM
-- ============================================================================
local AimColLeft = uiInstance.CreateGroupbox(uiInstance.Pages["Aim"].Left, "Aim Settings")
local AimColRight = uiInstance.CreateGroupbox(uiInstance.Pages["Aim"].Right, "Automation & FOV")

-- Аимбот (Аккордеон со всеми настройками)
local AimFeat = AimColLeft:AddFeature("Aimbot Assistance", true, function(v)
...
-- FOV Circle (Аккордеон с полным RGB)
local FovFeat = AimColRight:AddFeature("FOV Circle Indicator", true, function(v)
...
-- Triggerbot (Аккордеон со всеми параметрами)
local TrigFeat = AimColLeft:AddFeature("Auto Triggerbot", false, function(v)
...
-- // 2. ВКЛАДКА: VISUALS
-- ============================================================================
local VisLeft = uiInstance.CreateGroupbox(uiInstance.Pages["Visuals"].Left, "3D In-World Indicators")
...
-- 3D Chams (Полный набор: режимы, материалы, Color1 и Color2)
local ChamsFeat = VisRight:AddFeature("3D Player Chams (CS2)", false, function(v)
...
ChamsFeat:AddSlider("Effect Speed", 0.1, 5, 1.0, 0.1, "x", function(v) visualInstance.State.chamSpeed = v end)
...
local WorldRight = uiInstance.CreateGroupbox(uiInstance.Pages["World"].Right, "Atmosphere & Post-Effects")
...
CloudsFeat:AddSlider("Cover", 0, 1, 0.6, 0.05, "", function(v)
    worldInstance.State.cloudsCover = v
    worldInstance.UpdateClouds()
end)
CloudsFeat:AddSlider("Density", 0, 1, 0.7, 0.05, "", function(v)
...
-- Bloom & Motion Blur
local PostFeat = WorldLeft:AddFeature("Bloom & Motion Blur", false, function(v)
    worldInstance.State.bloomEnabled = v
    worldInstance.UpdateBloom()
end)
PostFeat:AddSlider("Bloom Intensity", 0, 5, 1.5, 0.05, "", function(v)
    worldInstance.State.bloomIntensity = v
    worldInstance.UpdateBloom()
end)
PostFeat:AddSlider("Bloom Size", 0, 56, 28, 1, "", function(v)
    worldInstance.State.bloomSize = v
    worldInstance.UpdateBloom()
end)
PostFeat:AddSlider("Bloom Threshold", 0, 2, 0.3, 0.05, "", function(v)
    worldInstance.State.bloomThreshold = v
    worldInstance.UpdateBloom()
end)
PostFeat:AddToggle("Motion Blur", false, function(v)
    worldInstance.State.motionBlurEnabled = v
end)
PostFeat:AddSlider("Blur Intensity", 0.1, 3, 1.0, 0.05, "", function(v)
    worldInstance.State.motionBlurMultiplier = v
end)

-- Чамсы на руки
local ArmsFeat = WorldLeft:AddFeature("Self Arms Material", false, function(v)
...
local WeatherFeat = WorldRight:AddFeature("Sky Weather & Particles", false, function(v)
    if not v then
        worldInstance.State.weather = "None"
...
WeatherFeat:AddSlider("Particle Density", 10, 300, 100, 10, "p/s", function(v)
    worldInstance.State.weatherDensity = v
    worldInstance.UpdateWeather()
end)

-- Синематика (SunRays & DoF)
...
CinemaFeat:AddSlider("Sun Rays Intensity", 0, 1, 0.3, 0.01, "", function(v)
    worldInstance.State.sunRaysIntensity = v
    worldInstance.UpdateCinematics()
end)
CinemaFeat:AddToggle("Depth of Field", false, function(v)
    worldInstance.State.dofEnabled = v
    worldInstance.UpdateCinematics()
end)
CinemaFeat:AddSlider("Focus Distance", 5, 100, 15, 1, "m", function(v)
    worldInstance.State.dofFocusDistance = v
    worldInstance.UpdateCinematics()
end)

-- Чамсы на оружие
local WeaponFeat = WorldRight:AddFeature("Weapon Material", false, function(v)
    worldInstance.State.weaponEnabled = v
end)
WeaponFeat:AddSegmented("Material", { "NEON", "FORCEFIELD", "GLASS", "FOIL", "PLASTIC" }, 2, function(opt) worldInstance.State.weaponMaterial = opt end)
WeaponFeat:AddSegmented("Color Mode", { "STATIC", "PULSE", "RAINBOW", "GRADIENT" }, 1, function(opt) worldInstance.State.weaponColorMode = opt end)
WeaponFeat:AddSlider("Speed", 0.1, 5, 1.0, 0.1, "x", function(v) worldInstance.State.weaponSpeed = v end)
WeaponFeat:AddSlider("Color 1 - Red", 0, 255, 0, 1, "", function(v) worldInstance.State.weaponColorR = v end)
WeaponFeat:AddSlider("Color 1 - Green", 0, 255, 255, 1, "", function(v) worldInstance.State.weaponColorG = v end)
WeaponFeat:AddSlider("Color 1 - Blue", 0, 255, 255, 1, "", function(v) worldInstance.State.weaponColorB = v end)
WeaponFeat:AddSlider("Color 2 - Red", 0, 255, 255, 1, "", function(v) worldInstance.State.weaponColor2R = v end)
WeaponFeat:AddSlider("Color 2 - Green", 0, 255, 0, 1, "", function(v) worldInstance.State.weaponColor2G = v end)
WeaponFeat:AddSlider("Color 2 - Blue", 0, 255, 255, 1, "", function(v) worldInstance.State.weaponColor2B = v end)
WeaponFeat:AddSlider("Transparency", 0, 1, 0, 0.05, "", function(v) worldInstance.State.weaponTransparency = v end)

-- Атмосфера, Тинт и Туман
local AtmGroup = uiInstance.CreateGroupbox(WorldRight, "World Atmosphere")

local CCFeat = AtmGroup:AddFeature("Color Correction", false, function(v)
    worldInstance.State.colorCorrectionEnabled = v
    worldInstance.UpdateColorCorrection()
end)
CCFeat:AddSlider("Saturation", -1, 2, 0.35, 0.05, "", function(v)
    worldInstance.State.saturation = v
    worldInstance.UpdateColorCorrection()
end)
CCFeat:AddSlider("Contrast", -1, 2, 0.15, 0.05, "", function(v)
    worldInstance.State.contrast = v
    worldInstance.UpdateColorCorrection()
end)

local TintFeat = AtmGroup:AddFeature("World Tint", false, function(v)
    if not v then
...
TintFeat:AddSlider("Tint - Red", 0, 255, 255, 1, "", function(v)
    worldInstance.State.worldTintR = v
    worldInstance.UpdateWorldColor()
end)
TintFeat:AddSlider("Tint - Green", 0, 255, 255, 1, "", function(v)
    worldInstance.State.worldTintG = v
    worldInstance.UpdateWorldColor()
end)
TintFeat:AddSlider("Tint - Blue", 0, 255, 255, 1, "", function(v)
    worldInstance.State.worldTintB = v
    worldInstance.UpdateWorldColor()
end)

local MapTransFeat = AtmGroup:AddFeature("Map Transparency", false, function(v)
    worldInstance.State.mapTransparencyEnabled = v
    worldInstance.UpdateAllMapParts()
end)
MapTransFeat:AddSlider("Transparency Level", 0, 1, 0.5, 0.05, "", function(v)
...
AtmGroup:AddSlider("Fog Density", 0, 100, 30, 2, "%", function(v)
    worldInstance.SetFog(v)
end)

AtmGroup:AddButton("Clear All Fog", function()
    worldInstance.ClearFog()
end)

AtmGroup:AddButton("Reset World Color", function()
    worldInstance.State.worldTintR = 255
    worldInstance.State.worldTintG = 255
    worldInstance.State.worldTintB = 255
    worldInstance.UpdateWorldColor()
...
-- // 4. ВКЛАДКА: SETTINGS
-- ============================================================================
local SetLeft = uiInstance.CreateGroupbox(uiInstance.Pages["Settings"].Left, "HUD Overlays")
...
local ListSettings = SetLeft:AddFeature("Active Binds HUD", true, function(v)
    settingsInstance.State.featureListEnabled = v
    settingsInstance.UpdateFeatureListUI()
end)
ListSettings:AddToggle("Show Active Only", true, function(v)
